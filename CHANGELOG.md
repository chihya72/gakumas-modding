...
- **以前做的 .blend**：当初手动镜像过一次，现在那次镜像成了多余的 → 左右装反闸门会拦下并
  点名这种情况。建议**回到未镜像的源模型重做**，并回阶段 1 重新导入参照体（旧参照体的左右
  和现在相反，劈权重和跨节过渡带都拿它当基线）。参照体上打了约定戳，旧的会在导出时报黄条；
...
划线标准改成**有没有实机后果的证据**，不是「能不能查出来」。下面四道降为黄条：

| 降级的 | 为什么不该硬拦 |
|---|---|
| 一行横跨多个骨族还下物理指令 | 这是对作者意图的**猜测**，不是确定缺陷 —— 作者可能真想让这一组一起动。已误报过（一个蝴蝶结被判成横跨 5 族） |
| 自建摇物链上没有带权重的骨 | 建了条不起作用的链是浪费，不是损坏 |
| 部件类型没有对应驱动器 | 后果是「这几根骨不会动」，可见、可恢复，不是废包 |
| 点了驱动器却一根没挂上 | 同上。提醒出来就不再是静默洞了，不需要拦 |

留下的硬拦（都有实机后果）：缺配置、面朝里、孤儿子树、左右装反、四肢/脊椎承重关节、
`reject`、未烘焙的 `bake`、新骨与游戏骨重名、未决定（带放行开关）、静止朝向 ≥45°。

**报错文案一并收敛**：只留「什么错了 → 后果 → 怎么改」。删掉阈值标定史、实测轶事、
「觉得不对再回来摆」这类把内部推理讲给作者听的话。现在每条 86~196 字。
