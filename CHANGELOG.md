...
- 修「手填目标骨的骨仍留在装饰骨集合」：装饰链的根是往上第一个非装饰骨，SCSP 的 `Hips_1` 手填目标后
  仍算装饰骨，它下面 13 条裙链 + 2 条飘带并成一条链，飘带的「刚性」吞掉裙子的「跟裙摆」，整条裙子钉到
  手填的那根骨上（实机：婚纱裙子是硬的）。手填过目标骨的骨现在归入身体骨，每条链各自决定。
- 修装饰骨兜底根写死 `Hips`：`hume-cstm-0010` 这类 Hips 没有权重的服装，Hips 不在可导出骨表里，面板显示
  一个填不进去的自动判定，清掉覆盖就报「顶点 N 没有可导出的配置档兼容权重」，作者只能乱填。现在兜底按
  Hips → Spine → 首个加权骨取。SCSP 源装到这类目标上，`Hips_1` 要手填 `Spine`（原版盆骨区权重就在 Spine 上）。
...
## 1.7.0 — 分叉躯干结构识别与稳定的骨骼处理队列

### 阶段 3 发型包改成「发型一栏 + 发饰一栏」

- 发饰以前是个折叠区，而且**只有三个贴图路径、没有材质槽** —— 发饰和发型是同等的两个
  Renderer、进同一个 bundle，折叠起来等于把一半工作藏起来；而发饰材质槽的 `材质类型`
  决定发饰描边常量（`_hairprop_export_colors`），面板里改不了，只能去 Scripting 里
  临时切 `gmi_author_object`。
- 现在两栏平级铺开，各有贴图 + 材质槽；描边只在发型那栏（发饰按材质类型写常量）；
  `生成游戏材质贴图` 挪到最下面，**一次生成两份**（发饰 t0 为空时只生成发型，不报错）。
- body 流程的顺序和内容没变。回归见 `tests/blender_ui_smoke.py`（两个列表各画各的对象）
  与 `tests/material_bake_blender_smoke.py`（一次点击两份 + 作者对象必须还原）。

### 抓帧选候选：不到 1000 顶点的发饰不再被整支砍掉

- `_select_main_candidate` 先按「有 VB1 且 ≥1000 顶点」缩池，再用调用方给的确切顶点数筛。
  发饰常在 900 上下（hski 的 `Geo_HairProp` 954、0082/0083/hair-0027 是 872），
  于是进不了池子，发饰被静默选成同帧的 `Geo_Hair`（16345 顶点），
  阶段 1 报 `指定发饰资源 … 是 954 顶点，但抓帧主发饰是 16345 顶点` 后整份配置档作废。
- 现在按确切顶点数没在池子里找到时，回到**全部**候选里找（仍要求有 VB1）。
  回归见 `tests/frame_profile_extract_smoke.py::test_extract_finds_sub_1000_vertex_hairprop`。

### `md_body1203_hq` 这类 Unity/Cygames 骨架也能按结构认

- 结构兜底不再假定 Spine 一定是 Hip 的子骨；支持 `Root -> Hip + Waist` 的兄弟分叉；
- 用两条腿重算上轴，避免长衣物链、手指或胸部挂件把人体方向带偏；
- 躯干中央链按垂直跨度选，手臂按同一胸椎上的左右成对分支选，头部不再钻进头发链；
- 左右轴不再受附件子树大小影响。已对 `md_body1203_hq.fbx` 验证，22 根 Humanoid 骨完整
  命中（含肩、颈和脚趾）。
- 修复同一手腕下的五条指链被套用裙摆片规则、错误合成 `Ring_01_L +14`：
  `Thumb_01_L`、`Iindex_01_L` 等常见格式会在确认手腕祖先后逐节映射到游戏指骨；即使
  指骨名完全不可读，兜底也只按每根手指链分别折叠，组标题使用真实链根。

### 「重新扫描待处理」与「列出全部」只切显示范围

- 扫描始终保存完整行集；两个按钮只切 UI 过滤，不再出现“列出全部后回不到四行”的状态污染；
- 手动映射后的行会从“待处理”视图消失，但仍保存在“全部”视图和导出数据里；
- “只看这些”改为单向进入待处理视图；不再允许用同一个 toggle 退出，返回完整列表统一点
  “列出全部”，避免两个入口看起来做同一件事；
- “清空”改名“重置映射”，并明确它只丢弃当前表单与手动覆盖，不修改模型、权重或外部 JSON；
- `+N` 骨组改为真正的行内树：三角直接在列表中展开子骨；点击子骨只改变选中项，不写映射；
  修改子骨的目标/刚性/摇物等字段才写逐骨覆盖，但子骨始终留在父组下面；
- 子骨清空目标并选回“自动”时删除逐骨覆盖，重新继承父组；父组设置仍一次作用于所有没有覆盖
  的成员，重扫后也按同一结构树恢复父设置与逐成员覆盖，不再炸成几十条顶级行；
- `逐骨不同` 明确算作未决定，展开逐骨覆盖后才会离开待处理；重置映射同时复位过滤器。

## 1.6.0 — 模板不用自己挑，作者手册改正

### 模板按目标资源名自动找

阶段 5 以前要作者从素材包的 908 个模板里挑出一个。他多半根本不知道抓帧抓到的那套衣服叫
`ttmr-cstm-0111` —— 而挑错一个不匹配的，**导出照样跑完**，出来的是坏包。Wiki 8.2 那句
「模板属于目标，不属于作者模型」的提醒就是为这个立的。

这一步不需要作者参与，因为答案是**算得出来的**：模板文件名由 `build_phase3_templates.py`
固定按 `template_<目标资源名>.bundle` 命名，而目标资源名早就在配置档的
`target.bodyResource` 里。目录 + 配置档 → 唯一确定的一个文件，没有匹配、没有打分、没有歧义。
这和之前那些踩过坑的启发式（按名字猜语义、按位置蹭骨）不是一回事。

- `R32 模板 bundle` 改名 `模板目录`，subtype 从 `FILE_PATH` 改成 `DIR_PATH`；
- **一个字段接受两种输入**：填目录就按资源名找，填文件就直接用。老 `.blend` 里存的是
  文件路径，别人单发一个模板给你也是这种情况 —— 不加第二个字段，也不做迁移；
- 找不到时报错点出缺的**文件名**（`template_mdl_chr_xxx_body.bundle`），作者拿它去素材包
  里补；以前是「请选择有效的 R32 模板 bundle」，查不下去；
- hair 同样成立：hair 配置档的 `bodyResource` 存的就是 hair 资源名（字段名叫 body 是历史
  遗留），发饰与发型共用同一个模板。

### 作者手册（Wiki）对齐到当前版本

Wiki 上一次改是为 1.3.0 写的，1.4.0（破坏性）和 1.5.0 都没同步，作者照着做会出错：

- **删掉全部手动镜像的教法。** 1.4.0 补上了坐标转换里缺的那次反射，正常导入的模型左右本来
  就是对的；照旧镜像会被「左右装反」闸门拦下。12.3 整节重写成「对齐与静止姿势」，
  11.13、12.2、12.7、5.4 的残留一并清掉，全 Wiki 现在零处提到镜像；
- **`映射到游戏骨` 这个控件 1.4.0 起就没有了**，改叫 `覆盖为`，语义也变了：身体骨由自动
  判定填好，这一栏只用来推翻它。7.2 补上「自动判定 / 覆盖为」两栏的分工和来路标注表；
- **补上 1.5.0 的结构识别**：骨名一个预设都对不上时按骨架形状认人形骨，这类行标
  「按骨架结构推断」且不折叠，认错了在 `覆盖为` 里改，必需骨认不齐就整批留空；
- **硬闸门清单补齐 1.4.0 新增的四道几何闸门**（面朝里、左右装反、孤儿子树、静止朝向）与
  自建骨重名；删掉「自建摇物链空链」—— 它 1.4.0 已降为提醒。8.4 另写明闸门为什么按这个
  顺序排：几何错了，后面所有关于骨和权重的数字都会跟着错；
- **自建半透明**：结论不变（不可交付），把过期的理由换成「仍在独立分支上探索，未交付、
  未实机验收」；
- 产品版本号不再写死（六处），Blender 4.5 LTS 从「也在测试范围内」改成明确支持。

`docs/wiki/` 与已发布的 GitHub Wiki 之前已经漂移，本版一并推平。**两边仍是手动同步，
没有自动化** —— 改 `docs/wiki/` 之后记得推 Wiki 仓。

### 顺带

- Release notes 的模板收成「本版变动：」+ markdown 列表 + 「包里有什么」+ Full Changelog；
  变动列表加 5 条上限（workflow 会拦）。CONTRIBUTING 第 5 节补上「一件事只写一条」——
  1.5.0 起初写了四条，其实是同一件事的四个侧面（改动本身、界面长什么样、"没变"、边界情况）。

## 1.5.0 — 骨名认不出来时，按骨架结构认

### 起因：认不出骨名不会报错，只会让所有尺子安静地报全绿

骨名映射靠八张预设表（MMD / mixamo / rigify / vrm / biped / auto-rig-pro / scsp / unity）。
名字落在这八家之外，`build_bone_remap` 一根也映射不上——**而这条路没有任何报错**：
对齐尺子、跨关节权重带、镜像闸门全都按游戏骨名索引，一根都匹配不上时它们各自报 0 根、
0.0°、0.0%，看上去满屏是绿的。而「骨名混乱、骨架也混乱」恰恰是最需要自动化的那类模型。

`gakumas_mi/topology_map.py` 早在 1.2 那批（2026-08-19）就为这个洞写好了，当时挂在
`unity_route.py` 上；1.3.0 收口 AB 路线时把 `unity_route.py` 删了，它的唯一消费者跟着没了，
模块留在包里但没有任何地方 import。本版把它接回骨骼扫描。

### 结构识别排在预设**之后**，只补空缺

映射阶梯现在是四档：`direct`（骨名字面就是游戏骨名）→ `strip_suffix`（去掉尾部 `_1`）
→ `preset`（八张表）→ **`topology`（按骨架结构推断）**。

- **只在预设漏了 Humanoid 必需骨时才启动**，启动后整体重算一次映射，让 body/accessory 分类、
  父骨兜底和 unmapped 清单都在一处重出，而不是逐项打补丁；
- **逐骨让位给名字**：预设命中的骨根本走不到第四档。另挡两样——预设已经认出来的源骨不改写，
  已经被占用的目标骨不去抢。名字是有据的，结构是推的；
- **全有或全无**：15 根必需骨（Unity 建 Humanoid 的下限）缺一根就整张表作废，不交半张。

判据不读任何骨名：整副骨架最长链的方向定「上」；同时挂着两条向下长链的那根骨是胯；
两条腿根部的连线定「左」；脊椎上横着分出去、走向以 ±左为主的两条是手臂；手臂挂上去的那根
是胸椎；锁骨靠「第一节明显比第二节短」认；头是脖子往下**直接子骨最多**的那根（头发、眼、
颌、耳都挂它下面）。手指不认——对姿势和体检都没有影响，而认错的代价比不认高。

### 面板如实标出来，而且不许折叠

结构识别推出来的行标 **「按骨架结构推断（骨名不认识）」**，并提示核对；这类行在
「只列未映射」的扫描里**永远列出来**。它是按骨架形状推的，正是作者最该亲眼扫一遍的那批，
藏进「已自动决定」里等于又回到静默。不对就照常在「覆盖为」里手填，覆盖压过自动判定。

一处行为要知道：预设 `direct` 命中一根骨、而结构识别认为它是别的骨时，**预设赢，那根必需骨
就空着**。空着会被承重关节闸门拦下并点名，不是静默。

### 回归

- `tests/blender_topology_fallback_smoke.py`（新，已进 CI 的 4.2/4.5 两档）：22 根 `b_00` 式
  废名骨、正常人体拓扑。先断言这副骨架**真的**打穿了八张预设表（否则测试是空的），
  再断言 22 根全部按结构认对、来源全部标成 `topology`、承重关节闸门通过；
  第三段把左大臂改成一个游戏骨名，断言 `direct` 赢、结构不抢；
- `tests/test_bone_classification.py` 补两例：结构猜测只补空缺、目标骨架里没有的丢弃；
  不传 `structural` 时逐字段等于老行为（兜底是加法，不许动已有结果）。

### 顺带

- 重复四处的「取骨架修改器」收成一个 `_armature_of()`；
- 骨骼扫描和导出改为共用同一个自动映射入口，免得以后再出现"一边接了另一边没接"；
- `CONTRIBUTING.md` §5 把 Release notes 从「禁止写什么」改成一份可照填的模板与自查清单——
  workflow 只校验形状（主题行 + 空行 + `- ` 列表），风格一直是空白页，所以逐版漂移。

## 1.4.0 — 闸门重新瞄准 + 修好坐标约定（**破坏性**）
...
### 闸门重新瞄准：从"数据自洽"转到"几何与权重"

> ⚠ **本节的闸门判据尚未实机复验。** 判据全部来自 2026-08-27 一次完整的 MMD → 学马仕
...
作者报告：全新重做的模型里 `胸上2.L` 被映射成 `RightBust1_S`，点「重新扫描」也修不掉。
两层根因，两条一起修。

- **位置匹配一律按链分档，不拿整组质心判左右**：`name_bust` 和 `follow_nearest` 拿**整组
  质心**去找最近的目标骨然后整组绑同一根。一个结构组里通常左右成对的件都在（实测
  `上半身2|L1` 一组 17 根，含 `胸上.L/.R`、`胸下.L/.R`），对称的质心落在 x≈0，`nearest`
  只能靠浮点尾数二选一 —— 左胸也绑到右边去了。逐骨判一点都不含糊：`胸上2.L` 离
  `LeftBust1_S` 74.5mm、离 `RightBust1_S` 135.0mm。裙摆那条路径早就改成「组里多条链就逐骨
  蹭、单链才按质心蹭」，胸和 `follow_nearest` 漏了，这次补齐 —— 判据统一成**组里有几条链**
  不是骨有几根（逐骨会把一条飘带扯断，质心会把左右成对的件绑到同一侧）。顺带核了 8 张预设
  表（mmd-standard / mixamo / rigify / vrm / biped / auto-rig-pro / unity-humanoid / scsp），
  左右全部一致、0 处接反 —— 这个 bug 只在位置匹配的兜底路径上。真模型验证：清掉旧代码预填的
  显式目标后重算，84 条可判左右的映射 0 处接反；
- **`target` 的语义从「这一行的决定」改成「作者的覆盖」**（表单里原来有第三份真相）：
  位置匹配的结果**从不进表单**，只在导出那一刻算，于是面板显示的不是导出用的；而 `target`
  一个字段同时装着「预设认出来的」「位置猜出来的」「作者手填的」，`origin` 只分 preset / 空，
  分不出猜的和手填的。再加上「重新扫描」按设计保住已填的行，**任何填错的值都永远修不掉，
  还会压住修好的算法**。现在 `target` 只在作者亲手填过时有值（一填就打 `origin="manual"`，
  清空退回空）；新增 `auto_target` 一栏，扫描时按「表单全空」现算，只显示、不参与优先级
  （它过期只影响显示，导出永远现算，面板和包不会对不上）；一行多根骨、组内自动判定不一致时
  如实显示「逐骨不同」，不假装是同一个值。装饰策略的四处判据仍只看**覆盖** —— 那几处的语义是
  「作者显式钉到身体骨」，拿自动判定当"已钉住"会把作者选的装饰策略整片吞掉；
- **面板判档改看生效值**：`ui.py` 有自己一份 `_row_state`，上一条只改了 `operators.py` 那份，
  于是面板拿覆盖（现在只有手填才有值）去判档，68 组全被标成「未决定」，而导出侧其实已经决定
  —— 面板和闸门给出两个相反的答案。现在两侧统一用「覆盖优先，否则自动判定」。实测重新扫描后
  67 direct + 7 merge、未决定 0 行。
- ⚠ **已经建过队列的 `.blend` 要点一次「重新扫描待处理」**：错误的目标是**烤进表单的**显式值，
  优先级最高，光升级插件不会生效。
...
- **导出报错内容为空**：14 处 `except Exception as exc: self.report({"ERROR"}, str(exc))`，
  而 `MemoryError` / `KeyError` / `IndexError` 这类异常 `str()` 是空的 —— 作者只看到一个空
  红叉，一点线索都没有。现在为空时退回异常类型名，并把完整 traceback 打到系统控制台；
- **DDS→PNG 转换在大图上 `MemoryError`**：列表推导给每个通道造一个 Python float 对象，
  8192² RGBA = 2.68 亿个 ≈ 8.6 GB（4096² 也要 2.1 GB，本来就在悬崖边上）。换成 numpy 后
  同一张图 1.07 GB，实测 8192² 现在导出正常。

### 文档与仓库

- **Wiki 重写**：按五阶段重新组织作者手册（28 个文件）；
- **删掉废弃的 unity-humanoid-avatar-sdk 研究路线**（305 个文件）及配套的 8 个工具与测试；
- 修复 CI 对本机资产的依赖。
