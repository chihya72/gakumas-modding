...
def base(target, step):
    ...
    d.text((630, 86), "v0.7.5", font=ft(12), fill="#9ca3ad")
    ...
    d.text((436, 168), "展开面板", font=ft(15), fill="#b9bec8")
...
def skinning():
    im, d = base("发型（含配套发饰）", "② 绑定模型")
    panel(d, 228, "② 绑定作者模型", 360)
    d.text((154, 272), "先在 3D 视图中激活发型或发饰作者网格", font=ft(14), fill="#cbd0d8")
...
def texture():
    im, d = base("发型（含配套发饰）", "③ 准备材质")
    panel(d, 228, "③ 指定导出贴图", 360)
    d.text((154, 270), "发型 / 配套发饰贴图可分别填写", font=ft(14), fill="#ffbd70")
    field(d, 304, "发型基础色 t0", "T_madoka_hair_D.dds")
    field(d, 384, "发型混合遮罩 t1", "T_madoka_hair_MSK.dds")
    field(d, 464, "发饰基础色 t0", "T_madoka_hairprop_D.dds")
    checkbox(d, 154, 548, "缺 t1/t4 时使用中性贴图", True)
    panel(d, 612, "创建预览材质模板", 86)
    button(d, 646, "创建预览材质模板")
    panel(d, 724, "材质槽设置：类型 / 透明路径", 86)
    panel(d, 812, ">  替代流程 · 只导出一张贴图", 66)
    d.text((154, 916), "hairprop 不显示 body 专用原生 co", font=ft(13), fill="#aeb4bd")
    return im

def export():
    im, d = base("发型（含配套发饰）", "④ 导出模组")
    panel(d, 228, "④ 模组信息", 360)
