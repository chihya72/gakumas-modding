"""D 包 + 只改一件事：把 Bag/Chain/Key/Spine_Bow 从 skirt 链里拿出来，判成 ribbon。

对齐逻辑逐字照抄 batch7_sample2a_rebuild_preserve_accessories.py（D 的脚本），
唯一的差别来自 inputs/physics-override.json 里新加的 swingCategories —— 所以
新包和 D 的 sidecar 除了 swingCategory / swing 参数 / swingChains 之外应当完全一致，
这也是验收判据（见 compare_sidecar_vs_d.py）。
"""
...
physics_path = project / "inputs" / "physics-override.json.bak-2026-08-19"
package_root = Path(r"D:\GIT\gakumas-modding\mod-workspace\experiments\batch7-sample2a-hmsz-control-authoring")
package_id = "batch7-sample2a-hmsz-source-skirt"
template = Path(r"D:\GIT\gakumas-modding\mod-workspace\libraries\templates\template_mdl_chr_hmsz-cstm-0059_body.bundle")
...
export_result = bpy.ops.gmi.export_bundle_source(also_patch=True)
print("EXPORT_RESULT", export_result)
print("EXPORT_SUMMARY", json.dumps({
    "packageRoot": str(package_root),
    "package": str(package_root / package_id),
    "bundle": str(package_root / package_id / f"{package_id}.bundle"),
