"""从 `authoring - 副本 - 副本.blend` 出一份符合当前契约的包。

physics-override.json 保持原样（不加 swingCategories）——作者在 blend 里的 123 行表单就是
权威。唯一的新变量是闸门 9（undecided 默认拦），这份包靠显式放行通过，放行会写进 sidecar。
...
physics_path = project / "inputs" / "physics-override.accessory-swing.json"
package_root = Path(r"D:\GIT\gakumas-modding\mod-workspace\experiments\batch7-fuku-accessory-swing-2026-08-19")
...
scene.gmi_allow_undecided = True   # 闸门 9：这份包还没逐组决定，显式放行并留痕
