...
> **发型道具 = 发型 + 可选发饰两个部件。** 游戏内只有「发型」选择，没有单独的发饰选择；
> 每个 `mdl_chr_*_hair` 资源包同时含发型网格 `Geo_Hair` 与发饰网格 `Geo_HairProp`，
> 装备发型即两者一起渲染。它们是两个独立 drawcall（不同于 body 的 `m_bdyco`
> 同网格第二材质段），但由**同一个发型 profile 和同一套四步 UI** 管理。顶层始终选择
> 「发型」；需要处理 `Geo_HairProp` 时勾选「制作发饰（可选）」。
> 两个 JSON 资源库也可以从同一批 `_hair` 包分别导出（`--mesh-name Geo_Hair` /
...
- 选项 2：填对应的 `网格 JSON 资源库`（body 含 `Geo_Body`；发型库含 `Geo_Hair`，需要发饰时还应含 `Geo_HairProp`）；
...
把「制作目标」设为 **发型** 并勾选 **制作发饰（可选）** 后，步骤②提供两条替代路线：

- 硬质发饰：选中作者发饰，点 **刚体绑定到 Head_Hair**；插件会清除旧权重，把全部顶点设为
  `Head_Hair=1.0`，导出后只随头部整体移动/旋转；
- 需要摆动/形变的软发饰：展开 B 路线并点 **从参考模型传递权重**，随后复核高风险顶点。

不要连续执行两条路线：刚体按钮会清除之前传递或绘制的权重。
...
#### hair 组件的贴图 / 顶点色语义（0.7.4，m_hair_21_001 实机验证）

hair 与 body 语义不同，制作目标选「发型（hair）」后插件自动按以下规则处理：

- **t1 PackedMask**：原版 hair t1 是全图常量 ≈ `(R=67, G=32, B=0, A=0)`。
  **A 不是 body 的 AO**——写 255 会让未替换的原版 t4 漏出来。烘焙时选「头发」
  材质类型即得该常量；未覆盖区/中性贴图也自动走 A=0 的 hair 常量。
- **t4 ShadeColor**：`RGB = 基础色(线性光) × 冷阴影乘数 (0.378, 0.367, 0.474)`，
  A=0。非 body 的单一 darken——蓝通道乘数更大，阴影整体偏冷。
- **顶点色 COLOR**：
  - 描边色 nibble `(R高, R低, G高)/15` 是**全网格常量**（非 body 的逐顶点基础色
    曲线，逐顶点合成会让暗部塌成绿色）。导出面板按发色明度选「发型描边色档」：
    深色发 `(0,0,1)` / 粉红发 `(1,0,0)` / 金浅色发 `(4,2,1)` / 纯黑 `(0,0,0)`；
    宁小勿大（描边宁暗勿亮）。
  - `G 低 nibble`（ramp 行）、`B`（0~15 细宽度）、`A`（144/0 高光掩码）由插件从
    **带权重参考网格**的 COLOR 最近邻拷贝——导出 hair 时场景里必须保留参考模型。

勾选「制作发饰（可选）」时顶点色同属常量描边语义，但发饰几何与参考不重叠，
不走参考拷贝，改为**按材质槽的「材质类型」写常量**（原版发饰实测）：
`metal` → 灰描边 `(3,3,3)` + 高光掩码 A=144；其余类型 → 黑描边 `(0,0,0)` + A=0；
宽度 B 统一 8。金属件请把材质槽类型标成 `metal`；亮色布件（白花等）想要灰描边
