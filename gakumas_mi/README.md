...
body/hair/hairprop 分支均在下文说明。当前能力边界见
...
> **发型道具 = 发型 + 发饰两个部件。** 游戏内只有「发型」选择，没有单独的发饰选择；
> 每个 `mdl_chr_*_hair` 资源包同时含发型网格 `Geo_Hair` 与发饰网格 `Geo_HairProp`，
> 装备发型即两者一起渲染。它们是两个独立 drawcall（不同于 body 的 `m_bdyco`
> 同网格第二材质段），**各建各的配置档、各出各的模组包**：完整替换一个发型时，
> 用同一次抓帧分别以「发型」「发饰」为制作目标走两遍流程，两个包同装即可。
...
- 选项 2：填对应的 `网格 JSON 资源库`（body 含 `Geo_Body`，发型含 `Geo_Hair`，发饰含 `Geo_HairProp`）；
...
把「制作目标」设为 **发饰（hairprop）** 后，步骤②提供两条替代路线：
...
制作目标为「发饰（hairprop）」时顶点色同属常量描边语义，但发饰几何与参考不重叠，
