...
> 同网格第二材质段），但由**同一个发型 profile 和同一套四步 UI** 管理。顶层始终只选择
...
把「制作目标」设为 **发型** 后，步骤②始终提供配套发饰的处理入口；是否实际制作发饰由作者网格决定：

- 硬质发饰：选中作者发饰，点 **刚体绑定到 Head_Hair**；插件会清除旧权重，把全部顶点设为
  `Head_Hair=1.0`，导出后只随头部整体移动/旋转；
- 需要摆动/形变的软发饰：激活发饰网格后点 **传递发饰权重**，随后复核高风险顶点。

只有发型作者网格时，导出只替换 hair 并保留配套发饰；同时仍使用 hairprop 特征精确限定目标。
如果同时准备发饰作者网格，点击一次 **校验并导出模组**，插件会自动合并为一个完整包。
不要在管理器里拆开启用 hair/hairprop 两个包。

### 4. 导出模组

- 推荐直接点击 `校验并导出模组`；
- 如需排错，展开“高级 / 分步导出”后点击 `仅校验当前网格`；
