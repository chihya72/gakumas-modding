...
- **Release notes 正文取自 tag 注解，不复制 CHANGELOG。** Tag 注解保留一行主题供 Git
  显示，workflow 只把后面的变动列表渲染成简短简介：

  ```text
  <主题：一句话概括本版，不写版本号、不加句号>

  - <改动一：用户能直接理解的短句>
  - <改动二>
  ```

  GitHub Release 最终固定显示为：
  ...
  ## 包里有什么
  <这个 zip 里有什么、怎么装 —— workflow 写死，每版一模一样>

  **Full Changelog**: <上一 tag>...<本 tag>
  ```

  **「包里有什么」和 Full Changelog 由 workflow 自动补，不写进 tag 注解，也不要从
  workflow 里删**：它讲的是这个 zip 怎么用，与本版改了什么无关，作者第一次下载全靠它。

  写之前核一遍：

  1. 每条只说一件用户能感知的变化，不写实现过程；
  2. **一件事只写一条**。1.5.0 起初写了四条，其实是同一件事的四个侧面 ——
     ①改动本身 ②它的界面长什么样 ③"名字认得出来的模型不受影响"（这是没变）
     ④"认不全时留空"（这是边界情况）。②③④ 都不是变动，属于 wiki 和 CHANGELOG。
     条数不是凑出来的：**这一版真改了几件事就写几条**，一件就一条；
  3. **一条一行、30 字以内，最多 5 条**（workflow 会拦）。一条里出现分号或「，还会…」，基本就是塞了两件事；细节留给 CHANGELOG；
  4. **不出现**：文件路径、`.py`、函数名、内部字段、commit hash、测试统计、仓库内部链接、
     "重构 / 收敛 / 收口"这类只有维护者懂的词；也不要写成对仗工整的标语 ——
     "宁可不认也不给半张错表"读着顺，但没告诉作者认出了什么、去哪改；
  5. 破坏性变化需要在同一条里说明旧文件怎么处理。
