...
- **Release notes 正文 = tag 注解**，不是 CHANGELOG。workflow 只校验*形状*
  （首行主题 + 空行 + 至少一条 `- `），**风格从来没有规矩，所以逐版漂移过**——
  0.9.3 写症状、1.1.0 写功能名（漏了 `swingChains` 这种内部字段）、1.3.0 只有三句概括、
  1.4.0 又回到症状 + 迁移。下面这份模板就是那条缺掉的规矩，照填，别重新发明：

  ```text
  <主题：一句话说本版最大的那件事，≤30 字，不写版本号、不加句号>

  - ⚠ 破坏性：<变了什么> + <老文件怎么办>          ← 没有破坏性变更就整条删掉
  - <改动一：作者看得见的现象，不是模块名或函数名>
  - <改动二>
  - ⚠ 已知问题 / 未实机复验：<有就写，没有就删>
  ```
  ...
  1. **每条都从作者的视角写现象**——"装饰件炸成贯穿角色的尖刺"可以，
     "新增 swingChains 与分叉链闸门"不行（`swingChains` 是 sidecar 内部字段名）；
  2. **3–7 条**。多于 7 条说明在写 CHANGELOG，回去合并；
  3. **⚠ 只留给两件事**：破坏性变更（必须带迁移指引）和已知问题/未实机复验；
  4. **不出现**：文件路径、`.py`、函数名、commit hash、测试统计、仓库内部链接、
     "重构 / 收敛 / 收口"这类只有维护者懂的词；
  5. 主题行会被渲染成 `Gakumas Mod 制作工具 **X.Y.Z** — 主题`，所以主题本身别再带产品名。

  正文之后由 workflow 固定补上 `包里有什么` 与 Full Changelog，每版一模一样，不用写。
