...
> ⚠️ **「自建摇物链」当前不保证能动**（0.9.3 状态）。最近一次实测：链建好了、日志全绿，
...
| `材质槽 N 缺少 t0 基础色贴图` | 有材质槽标了「原生 co」，但 **co 那三栏的 t0 是空的**（co 的 t0 不会回退借用 body 的） | 补上 co 自己的 t0；这件衣服其实不需要透明件的话，把该槽的 `渲染材质` 改回「不透明」。**插件 0.9.3 之前**还有一种情况：目标服装有 3 段时，即使你一个 co 槽都没标也会报这句——升级到 0.9.3 即可 |
