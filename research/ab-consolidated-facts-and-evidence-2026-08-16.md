...
## 4b. 跨游戏源样本 chs-sucu-00 → hmsz-cstm-0059（2026-08-21 实测）

一次做出来撞出七条，**只有 1–3 是 IP 源特有的，4–7 是插件通用缺陷**（第 6 条量到了但没改）。
路线与这一批的产出清单见路线文档 §H。

同一套 VL 中间件、骨名与拓扑一一对应，**但体型不是一个尺寸**：

```text
              源 chs-sucu   目标 hmsz-0059   差
Head y          1.248        1.372        124 mm
LeftHand      (-0.459,1.151) (-0.529,1.265) 134 mm
Hips y          0.853        0.938         85 mm
87 根共有骨：最大 271.7 mm，中位 133.6 mm；整体等比系数 1.1028（按中位比例）
```

三条可复跑的结论：

1. **「lossless 免的是位置」在体型差 10% 时不成立。** 每根骨的几何块被各自搬到原版骨位置，
   关节处撕裂 —— 画面上就是**头飘在脖子上方、手够不到手骨**。必须做 P2 对齐
   （定版脚本随样本保存在 `mod-workspace/mods/work/chs-sucu-00_body_ab-2026-08-21/`：整体等比 + 逐骨摆到
   目标静止矩阵 + 骨架修改器把形变烘进网格，实测最大骨差 179.3mm → 0.000mm）；
2. **只做逐骨、不做整体等比**，10% 身高差全由关节的权重过渡带吸收（实测总边长只涨 0.3%、
   身高涨 9.5%）。先等比再逐骨，残差从 179.3mm 降到 94.4mm 再归零；
3. **等比之后残差 >30mm 的骨恰好是那 14 根裙摆摇物骨（82–221mm），其余 62 根带权重骨全 ≤26mm。**
   同名不等于同一件裙子 —— 按名字并过去会把包臀裙拉成目标那条蓬裙的形状。它们要走自建摇物骨。
   配套修掉一个静默洞：作者在表单里点了物理策略、又没填目标骨的骨，以前**名字命中目标骨架**
   时名字仍然赢（`operators._resolve_source_bone_remap`），作者的决定被吞掉。

4. **自建骨与目标骨架重名 = 硬冲突（契约 §4.1 的防撞名不是风格问题）。** 上一条把 14 根源
   裙摆骨改成自建骨，名字却没改；目标 dress-2219 自己也有 `RightFrontSkirt1_S`，导出器于是
   拿**目标那根**的 local 变换填 renderer 的 `bones[]`（`localPosition` 变成 `0`＝胯中心），
   而 `newBones` 段里记的仍是我们自己的位置 —— 两处打架，画面上那片前裙板绕胯中心摆，
   表现为"先向上折再垂下来"。加前缀后 `boneCount` 从 170 回到 184（156+28），
   世界位置与源×等比只差 3mm。闸门：`core.new_bone_name_collision_error`（导出前硬拦）。
   连带：**无权重的锚点父骨（`*_Repulsion_A`）也要跟着保留源静止姿势**，否则整条源裙摆链
   被拖到目标那件衣服的锚点上（实测裙板被抬高 85mm、前移 32mm）。
   连带二：`verify_ab_package._side_of` 只看名字开头，前缀一加就把自建骨全判成 unclassified，
   「左右不对称」那条 bug 指纹会哑掉 —— 已改成全名里找 left/right。

5. **合成链尾的方向必须按几何来，不能沿骨的局部轴。** `build_source_extra_bones` 原来把
   链尾硬写在 `[0, 0, -骨长]`（Blender 骨的局部 -Z + 默认骨长 55mm）。源骨的局部轴是任意的，
   实测 chs-sucu：

   ```text
   链根                        链尾世界方向        修好后
   chssucu_LeftFrontSkirt1_S   (-1.00, 0.00, -0.01) 正侧向   → ( 0.17, -0.97,  0.17) 朝下
   chssucu_RightFrontSkirt1_S  (-1.00,-0.03,  0.01) 正侧向   → (-0.21, -0.95,  0.23) 朝下
   LeftBackWing2_S             (-0.38,-0.93, -0.03) 朝下     → (-0.70,  0.65, -0.30)
   RightBackWing2_S            (-0.38,+0.93, +0.03) 朝上     → ( 0.69,  0.66, -0.30) 左右终于对称
   ```

   链尾定义"这一节朝哪儿"，解算就按它把这一段拽过去 —— 画面症状是**裙片整体偏向一侧**
   （静止网格是对的：裙板质心与源只差 3–9mm，所以查网格查不出来）。
   源模型自己的链尾在局部 **-X**（`CenterBackTail5_S_End` 实测 221mm），也不是 -Z，
   所以这个轴不能猜。改成把链尾放到**该骨主导顶点的质心**（`operators._dominant_group_tip_offsets`），
   任何源都成立；交叉验证：尾巴链尾几何算出 219.5mm，源自己授权的是 221mm。
   回归 `tests/test_bundle_source_contract.py::test_chain_tip_follows_the_geometry_not_the_bone_axis`。

6. **顶点 COLOR 的 RampAdd 行 / rim 档跨游戏不同族（已量到，但 2026-08-21 作者判定画面没问题，
   保持"保留源值"不改）。** 抓帧 `FrameAnalysis-2026-08-21-171023` 的 body `t7` 就是
   `_RampAddMap`（128x16）：row 0–1 黑、2–4 (27)、**5–7 (152,105,50) 橙棕**、8–10 (51)、
   11–13 (40)、14–15 (49,56,79)。行由 `COLOR.G` 低 nibble 选，原版同一件 body 只用
   `{0,3,6,9,12,15}`，而保留源值的包 16 行全用上；`COLOR.A` 原版全是 16 的倍数、源是任意值。
   要动的话预设里有目标游戏的值（skin 行 0 / leather_shoe 6 / leather_plastic 12、A ∈ {0,144}）。

7. **归组的「链长 ±1」不能跟上一条链比，类别不能按组发。** 归组规矩是「同锚点 + 链长相同 ±1」，
   但实现里 ±1 是跟**上一条链**比，长度 1,2,3,4,5 会像多米诺一样串成一组：

   ```text
   改前  Hips|L1  24 根 / 12 条链   ← 裙摆 + 尾巴 + 两条飘带 + 前裙饰 全在一起
   改后  Hips|L1  13 根 /  9 条链
         Hips|L3  11 根 /  3 条链
         Spine2|L1 6 根 /  4 条链
   ```

   同时 `geometric_swing_categories` 把**一组一个类别**发下去（与 `cf12f94` 修的"物理指令按组发"
   是同一类错误，只是换了个字段）。已改成：**作者显式点的整组生效**（"一起选"的本意），
   **自动判的按链算**（链的垂向按这条链自己的根→尾算；"同锚点有几条链"仍取组级，
   一圈裙板本来就是靠这个认出来的）。链的切法抽成 `core.resolve_chains`，
   分组和类别两处同源。
   副作用：chs-sucu 的 14 根裙骨自动判定从全 `skirt` 变成 `skirt 7 / cloth 7`
   （后裙长链自成一组、3 条 < 一圈的 4 条）；那个包按"画面已确认"钉成显式 `skirt`。

另外两条与贴图/顶点色有关的（同一次）：

- IP 源必须先跑 `tools/process_ip_geo_body.py`（逐顶点分类 → 描边 nibble，保留 ramp 行/rim/A），
  见 `IP/06-ab-route-handoff/docs/ab-route-pipeline.md` §4；
- **UV 的 v 被多翻了一次**（`core.read_weighted_reference` 翻，导出端不翻回去）：任何从
  Mesh JSON 导进 Blender 再出包的模型，贴图都是上下颠倒着采的 —— 画面上就是"整身贴图错乱"，
  而 t0/t4 与源逐字节相同、查贴图查不出来。判据：chs-sucu 的 Head 皮肤顶点原样 v 采到
  `(228,193,179)` 肤色、翻转 v 采到 `(122,104,104)`。已改成原样（Unity 与 Blender 的 UV
  原点都在左下），回归 `tests/test_bundle_source_contract.py::test_weighted_reference_keeps_unity_uv_orientation`；
- 而 body 导出器原来**按材质槽写常量 COLOR**，一个 `m_bdy` 槽里的皮肤和布料因此拿到同一行
  ramp（实测 1093 个皮肤顶点按布料渲染 = 脖子/大腿发黑）。已加描边模式
  `SOURCE`「沿用源模型顶点色」，缺 COLOR 硬拦。
