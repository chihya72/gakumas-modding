...
- `导入 Profile 对象`：只选 Profile 目录和组件，就会同时导入抓帧参考、带权重原模型、
...
- `创建 Body 材质模板`：在 Blender 材质上记录 t0/t1/t4 语义，作为作者预览和导出提示；
- `校验并导出 Mod`：把“校验 → 选择原拓扑/带权重导出”合并为一个主按钮。
...
- `导入 Profile 对象` 可以隐藏 `原模型 JSON / 骨架 JSON` 的普通作者入口；
- 仍保留 JSON 字段作为高级/调试入口。
