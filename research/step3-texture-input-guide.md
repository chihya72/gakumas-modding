...
更新：2026-07-14
适用：GakumasMI 0.7.x 步骤③「材质槽设置」

这份文档只说明**作者模型需要准备哪些图像、在 Blender 哪个路径填写**。步骤③的路径不是游戏原始文件路径，而是作者自己制作的 PNG/DDS 文件路径。
...
填写 t0 后，如果没有现成 t1/t4，应使用「按材质生成 t1/t4」；如果只想先测试颜色，可以开启「中性 t1/t4」。不要让插件沿用目标角色原来的 t1/t4，否则新 UV 会采到旧衣服或旧发型的颜色块。
...
1. 把 t0 填到 `基础色 t0`；
2. 保持「中性 t1/t4」开启，或点「按材质生成 t1/t4」；
...
「按材质生成 t1/t4」必须分别激活 Hair 和 HairProp 作者网格执行。两者使用不同的临时输出，不会互相覆盖。
