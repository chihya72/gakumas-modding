...
> **状态（2026-08-02）：结论部分已废弃，从 `codex/il2cpp-route` 分支取回作历史记录。**
>
> 本文写于 2026-06-30，当时只有两条候选路线，**AB bundle 路线还不存在**。它比较的三条路线
> 里，学马那两条现在都不是主线了：
>
> | 本文的说法 | 现在 |
> |---|---|
> | 3DMigoto 是正式主线 | 0.9.0 起从插件整体移除，只剩抓帧工具 |
> | IL2CPP 是值得继续的研究路线 | 已彻底放弃，代码 2026-08-02 删除（见 [`pc-il2cpp-gmim-runtime-replacement.md`](pc-il2cpp-gmim-runtime-replacement.md)） |
> | ——（未考虑） | **AB bundle 才是主线**：把网格作为正规 Unity 资产交给引擎原生蒙皮 |
>
> 所以 **§6 能力边界对照表和 §7 战略判断整体作废**，别拿它做决策。§7 断言「IL2CPP 是学马里
> 唯一能绕回 Unity skinning 前的数据层的路线」——这句话被 AB 路线证伪了：AB 做的正是同一件事，
> 而且不需要进程内注入。
>
> **仍然成立、也是保留本文的理由**：§0–§2 论证了**为什么 GIMI/WWMI 那套照搬不到学马**——
> 米哈游系能改权重是因为 draw/shader 层真有可替换的 blend buffer 与 skeleton buffer，
> 而学马 body 是 CPU 蒙皮后才进 D3D draw，3DMigoto 抓到的 VB0 里根本没有
> `BLENDINDICES / BLENDWEIGHT`。这条是永久事实，也是当初否掉「照抄 GIMI」的原始依据。
> §5 的透明结论（必须复用原生 `m_bdyco`）同样仍然成立。
>
> 路线取舍的当前版本见 [`3dmigoto-vs-ab-route.md`](3dmigoto-vs-ab-route.md)。
