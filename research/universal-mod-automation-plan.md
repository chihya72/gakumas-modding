...
| 身体骨对照（SCSP / QualiArts） | ✅ | **A 实机** | fuyuko → `hmsz-cstm-0059` 重导后手部跟随大动作正常；装饰物理另按 B 级计算 |
...
| 骨骼映射表（UI 表单） | ✅ | **B 离线** | 用假 layout 跑过绘制、用脚本在对齐好的 blend 上验过映射生效与 JSON 往返；**没有人在真 Blender 面板里点过，也没有由它产出过进游戏的 mod** |
| 出门闸门（14 承重关节） | ✅ | **B 离线** | 脚本验过"MMD 过 / 改名成陌生骨架被拦 / 填完放行"；真作者会话里没触发过 |
| 装饰骨策略列 | ⚠ 半自动 | **B 离线** | 位置匹配实测 1/3 命中；策略列产出的映射与手写 JSON 逐条一致，**但那份手写 JSON 的实机效果本身也没验过**（脖子缎带/腰饰是否不再乱甩＝未知）|
| t1/t4 不再导成纯黑 | ✅ | **A 实机** | 2026-07-26 重导后 t4 从 78KB(纯黑) 变 3.4MB，游戏内颜色分层正确 |
| 肤色校正（t4.A mask 取值 + UV 足迹校色） | ✅ | **B 离线** | 数值对到 G/B 差 0.5% 以内；**校色后的 t0 没有进过游戏** |
| 移除 3DMigoto 导出路线 / UI 收三步 | ✅ | **B 离线** | 全套测试通过、反向断言 UI 无残留入口；0.9.0 代码已安装，但尚未用真面板完成一次 UI→导出→实机闭环 |
| 打包 | ✅ 免 Unity | **A 实机** | R32 模板 + UnityPy 补丁，多个 mod 进游戏验证过 |
| 对齐 / 删头 / 图集 | ❌ 不做 | — | **作者基本功，明确排除出插件范围**；这是端到端一键化真正的拦路虎 |
| 插件分发 | ❌ 阻塞 | — | 游戏侧插件改动仍只在本地且仓库含目录迁移/大量删除，尚未整理成可上游提交（§6.7 风险 1） |

**下一件事其实是「把 B 变成 A」**：装 `dist/` 最新包，用真 Blender 面板走一遍
（映射表点选 → 闸门 → 导出 → 进游戏），特别是确认①表单在真 UI 里可用②闸门不误伤
③装饰策略让脖子缎带/腰饰不再跟着袖子甩④校色后的肤色对得上。这一轮没做，因为改动是在
作者不在场的情况下连续做的。

**下一步 = 进入发布冻结（2026-07-27 定，取代此前的三条优先级）**

技术风险基本还完了，**剩下全是交付风险，而最急的一条是落盘**。

**当前"完整能力"不存在于任何一个 commit 里。** 基础 AB 导出已在 7/24 的 commit 里；真正没
落盘的是**之后那组组合能力**——崩溃根治、八家预设、UI 收三步、删 3DMigoto 路线。核对：
本仓库 28 项（tracked diff `+1934/−3233` + 6 个 untracked），main 停在 7/14、分支 head 7/24。
...
第一步必须**先判定这些删除的性质，再拆提交，绝不允许整体 commit**。
...
2. **装饰骨默认策略翻转**：位置匹配退出默认路径，默认改成跟源父骨，加 `胸`→`Bust*_S`
   名字规则（实测 1/3 → 3/3）。**必须在冻结前做完**，因为它改导出结果——
   反过来先验收再翻转，刚拿到的 A 级证据立即作废。属行为变更，动手前先确认。
...
> 所以不能笼统查"源骨名泄漏"，应查：**身体骨已映射；未映射的名称必须被声明为合法 `newBone`，
> 且有合法父级与物理策略**。§10 那张清单现在全靠人对照，脚本化后闭环里人只需要看画面。
...
> | 装饰骨物理归属 | 位置匹配实测 1/3 命中；靠 override 兜 | **命名和位置都决定不了，注定要人给一句话** |
...
**泛用解已落地(2026-07-25):`build_accessory_physics_remap` 重构为三层分类** —— **override(作者显式) > 语义名字规则 > 位置兜底**。策略:`integrate`(自己物理,飘带/蝴蝶结)/`follow_skirt`(蹭最近裙摆,花边)/`follow:<骨>`(蹭指定)/`rigid`(无物理跟父)。作者 override 走 `gmi_physics_override_file`(JSON `{骨名或前缀: 策略}`,前缀最长匹配),UI 面板「装饰物理覆盖」。不传 override 时行为等价旧版(源链→integrate、lace→follow_skirt、余者位置匹配),fuyuko 靠语义默认即可分类正确、无需 override。测试:`test_physics_override_precedence` + `test_lace_rides_skirt_not_nearest_leg`。
...
- [ ] 两个仓库提交边界理清（插件仓库那 330 个删除已判定性质），工作树干净，无 untracked 关键文件
- [ ] `pytest` + 两个 Blender headless 回归全绿，CI 通过
- [ ] 装饰策略翻转已落地；runtime 版本协议 + `buildId` 已生效；README 边界已写
- [ ] **RC 完成一次真实 UI → 导出 → 实机闭环**，发布关键 B 项全部转 A
- [ ] 开头状态表里没有任何一行还是"发布关键 + B"
...
- [ ] **游戏侧插件已上游合并，或自带 DLL 一并分发**（bundle 对改版 ModRuntime 是硬依赖，
      只发 Blender 插件等于没发）
- [ ] 导出器 ↔ runtime **版本校验实机确认**：版本不匹配明确报错，而不是产出诡异的包
      （协议本身在 RC1 之前就要完成，见「下一步」第 3 条）
- [ ] 两类源各一个走完新流程的 A 样本：**QualiArts 同骨架** + **MMD 外部源**
- [ ] ZIP / DLL / bundle 的 hash **与 `buildId`** 及对应日志留档

### RC 闭环必须一次清掉的 B 项（照单执行）
1. 从 **ZIP 全新安装**插件并**彻底重启 Blender**（不是覆盖文件——安装路径是拷贝不是软链）
2. 真面板**扫描、修改、保存、加载**骨骼映射表
3. **故意制造缺失承重骨**，确认闸门拦下并点名；修正后确认放行
4. 用**翻转后的装饰策略**导出
5. 游戏内确认：加载、身体动作、手部、裙摆/飘带物理
6. 确认**校色后的 t0**
7. 保存 **ZIP / DLL / bundle 的 hash、`buildId` 与对应日志**，避免以后不知道测的是哪个包
...
以下只列当前已经存在的 B 级项；排序按“对首个可交付版本的阻塞程度”排列，难度和可实现性是
在现有代码、现有 Blender/游戏环境下的估计。A 级必须有真实 Blender 或游戏证据，离线测试不能替代。

| 排序 | B 级项 | 转 A 的最小证据 | 优先级 | 难度 | 可实现性 | 依赖 / 备注 |
|---:|---|---|---|---|---|---|
| 1 | 真 Blender 面板的骨骼映射表 | 从 RC ZIP 全新安装；扫描、修改、保存、加载；用表单产出一个进游戏的 mod | P0 | 中 | 高 | 与整条 UI→导出→实机闭环合并验收 |
| 2 | 14 承重关节闸门 | 真作者会话中故意缺映射时被拦，补齐后放行，并确认提示准确 | P0 | 低 | 高 | 不需要新算法，只需真实触发一次 |
| 3 | 装饰策略与物理归属 | 先完成“跟源父骨”默认策略翻转，再用真实服装确认飘带/蝴蝶结/裙摆不乱挂 | P0 | 中 | 中 | 改变导出结果，必须在最终 RC 前完成；不要求手感完美 |
| 4 | 新骨链的最低限度实机行为 | 游戏中确认加载不崩、链条被注册、左右件有基本摆动；保留“手感未调完” | P0 | 中 | 中 | 与排序 3 同一轮验收；日志不能替代画面 |
| 5 | 校色后的 t0 | 用 RC 导出并进游戏确认肤色，不只看离线 G/B 数值 | P1 | 低 | 高 | 资源导出已完成，缺的是一次实机证据 |
| 6 | 逐张表打分选表 | 用至少一个真实模型跑完整选表逻辑，并保存命中/误选记录 | P1 | 中 | 中 | 不阻塞首个版本；表外模型仍有 UI 兜底 |
| 7 | 移除 3DMigoto 后的三步 UI | 新包安装、重启 Blender，完整走“配置档→材质→AB 导出”，确认没有旧入口和旧路径依赖 | P1 | 低 | 高 | 是排序 1–5 的验收外壳，不应另起长期开发线 |
...
工作树当前有未提交改动；接手时不要 `reset --hard` 或覆盖已有变更。

## 12. 一句话给接手人

**已进入发布冻结，只做「提交边界 → 翻转 → 版本协议/buildId → README → 核包工具 → RC → 闭环 → 分发」，别加功能（顺序与闸门见开头「下一步」与 §9.5/§9.6）。**
**AB 核心链路已有实机样本，0.9.0 候选包已生成；插件只保留 AB 导出路线，3DMigoto 仍用于抓帧。**
身体骨对照已有八家预设、作者点选表单和承重关节闸门，MMD 与 SCSP 有 A 样本，其他预设和真 UI 闭环
仍待验收。**下一接手动作是先整理两个仓库的提交边界并落盘，再按 §9.6 清发布关键 B；完成 Gate A
后才处理插件上游/自带 DLL 和 Gate B 分发。**
装饰骨物理归属仍需要人给一句话（实测位置匹配 1/3 命中），已有表单入口兜住；
把默认从"位置蹭"翻成"跟源父骨"能提到 3/3，但属行为变更，动手前先确认。
...
- **P3 的诚实账**：本次 3 组装饰骨，位置匹配只对了 1 组（`胸上2`→`Bust1_S`）；脖子缎带被蹭到
  夹克**袖子**摇物骨、腰部挂件被蹭到大腿皮肤摇物骨。改成「跟源父骨映射后的骨」能对 2 组，
  再加一条 `胸`→`Bust*_S` 的名字规则才 3 组全对。**位置匹配退出默认路径这件事仍未做**
  （行为变更，会影响已发布 mod 的可复现性），见 §6.5。
