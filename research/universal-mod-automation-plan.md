...
>
> `B→A（真实模型）` 是 §9.6 对“逐张表打分选表”的专项完成标记：使用真实导出模型的源骨名
> 跑完整选择器并保存全部命中数；它不等同于身体/物理的游戏 A 实机等级。
>
> **A 级实证来源**：本轮真实 Blender/游戏操作由作者完成，本助手负责核对产物、日志和记录；
> 不能用离线检查替代作者的实机验证。
>
...
**RC1 真实闭环已经执行，不需要因为旧 Mod 的历史协议问题重做当前闭环。** 真实 Blender 操作、
闸门结果、两套 Mod 的游戏结果、核包输出、DLL/ZIP/bundle hash 和对应日志已整理到
[`research/rc1-evidence-20260727.md`](rc1-evidence-20260727.md)；旧资产的协议迁移另记为兼容性工作。
...
| 游戏实机 | 已完成首轮 | 两套当前 Mod 已测试，游戏内未见本轮链路问题；结果与 runtime 关键日志已归档 |
| RC 产物 hash | 已完成 | ZIP、DLL、author/fuyuko bundle SHA-256、buildId 和核包输出已归档 |
...
**下一步 = 由作者手动推送插件仓库，再进入 Gate A（2026-07-27）**
...
4. **README 写死「通用性」边界**（§12 那句）。对内止血：定义不写死，"再加一张骨骼预设表"
   就永远看着像在推进。
5. **最小离线核包工具**（见下）。
6. **冻结，从确切 commit 打 RC1。**（候选产物与证据记录已归档。）
7. **走一次真实 UI → 导出 → 实机验收**，一次清掉全部发布关键 B 项（已执行并归档）。
8. **任何会改变 ZIP / DLL / bundle / sidecar 的修复，一律出 RC2 并重跑全流程**，
   不许在 RC 上原地打补丁——否则发出去的包和"验收通过"的包不是同一个。
   README 错字、验收记录补充不属此列，不必重跑。
9. **证据包已通过；插件仓库推送由作者手动完成**，随后合并 main、**ZIP 与 DLL 一起分发**。
...
> **"插件分发"与历史上的"B 级堆积"是两个不同的阻塞**：前者是**对外可用性**阻塞
> （不解决则能力对外为零）；后者在 RC1 真 UI→导出→实机闭环后已清空。当前不要继续生产
> 新的发布关键 B；后续开发属于 C 级预设回归或首发后的体验调优。
