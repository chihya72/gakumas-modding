...
| 装饰骨策略列 | ⚠ 半自动 | **B 离线** | 默认已翻转为源父骨；胸/Bust 走 `Bust*_S`，飘带/花边显式语义和 override 保留；新默认尚未进游戏验收 |
...
**已进入发布冻结；提交边界已落盘，策略翻转、版本协议/buildId、README、核包工具已完成代码实现，下一步是 RC → 闭环 → 分发。**
**AB 核心链路已有实机样本，0.9.0 候选包已生成；插件只保留 AB 导出路线，3DMigoto 仍用于抓帧。**
身体骨对照已有八家预设、作者点选表单和承重关节闸门，MMD 与 SCSP 有 A 样本，其他预设和真 UI 闭环
仍待验收。**下一接手动作是用本轮产物跑 §9.5 的真实 UI → 导出 → 实机闭环；完成 Gate A
后才处理插件上游/自带 DLL 和 Gate B 分发。**
装饰默认已从"位置蹭"翻成"跟源父骨"，另有 `胸`→`Bust*_S` 规则；当前仍是 B，必须用真实服装确认。
