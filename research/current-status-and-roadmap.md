...
| 透明材质 | ~75% | 路线已收敛到游戏原生 `m_bdyco` 第二材质段：`NATIVE_CO` 材质必须使用含 co section 的配置档；自建透明与几何侧处理均不作为方案。 |
...
- 材质属性：材质类型预设、`渲染材质`（不透明/原生co）、明暗 / 阴影色微调。
...
- **透明材质架构结论**：透明必须复用游戏原生 `m_bdyco` 第二材质段；主 `m_bdy` 不读取
  `_BaseMap.a` 做透明。详见 [`transparent-material-status.md`](transparent-material-status.md)。
...
| 进程内 IL2CPP Runtime 替换 Mesh 作为正式发布路径 | 维护/分发/崩溃风险高 | 仅保留为研究路线；正式路线仍是 3DMigoto。对比见 `pc-il2cpp-gmim-runtime-replacement.md` |
...
| 主 `m_bdy` 上自造透明或几何侧处理 alpha | 黑底/行为不等价/破坏几何 | 透明必须走原生 `m_bdyco` 第二材质段 |
...
### P3 — 透明材质（75% → 目标 90%）
- 继续打磨 `NATIVE_CO`：多服装 profile 校准、co section 多 pass 覆盖检查、导出前诊断与错误提示。
- 没有原生 `m_bdyco` section 的配置档不承诺透明；作者需改回不透明或换用含 co section 的抓帧。
