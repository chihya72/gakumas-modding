...
已达成并跨服装实机验证。一键即可对任意 body 生成完整配置档，配套 Mod Manager 已发布。
**项目已进入收敛/维护态**：核心 GPU 算法冻结，透明(P3)、蒙皮精修(P1)、多组件扩展(P4 脸/
头发/饰品)均已降优先级/暂缓，接受"单 body 换装 + Mod Manager"为当前交付边界。仅剩材质打磨
(P2) 与回归维护(P0) 为低强度进行项。
...
| 蒙皮转权（作者模型 → 配置档骨架） | ~85% | 全身最近表面（KDTree）自动传权稳定、自动限四权重/归一化、高风险顶点写入 `GMI_REVIEW_HIGH_RISK` 组。⚠️ **已确认高于参考基准**：参考项目 theherta4（GIMI/WWMI 家族）根本没有自动传权，只靠数字顶点组 + 手工权重笔刷；我们的自动传权 + 风险标记已超出。手指/宽袖/裙摆的引导式可视精修属"锦上添花"，**优先级降低、暂缓**，接受"自动传权 + 风险标记 + 手工复核"为当前边界。 |
...
| 产品化（Mod Manager / 发布；多组件已暂缓） | ~75% | **Mod Manager 已功能完备并随 `3dmigoto-gkms-v0.5.x` 发布**（Inno 安装包、应用图标）：扫描/启停/F10 重载/冲突检测/完整性校验、路径记忆、封面缩略图、拖拽安装（文件夹/zip）、打开目录；不做 d3dx 图形化设置页（改发 `键位说明.txt` + 默认 hunting=2 关绿字）。0.7.3 起 manifest `targets`=被替换的游戏资源名、强制预览图，正对上管理器展示。⚠️ 脸/头发/饰品等**其他部位多组件、ShapeKey、LOD 已降优先级/暂缓**；% 按当前边界（单 body 换装 + Mod Manager）计。规划见 [`mod-manager/docs/plan.md`](../mod-manager/docs/plan.md)。 |
...
### P1 — 作者蒙皮体验（~85%，**已降优先级 / 暂缓**）
- 参考项目 theherta4 都没有自动传权（只有数字顶点组 + 手工笔刷），我们的 KDTree 自动传权
  + `GMI_REVIEW_HIGH_RISK` 标记已在其之上；剩余的可视化精修属锦上添花，不主动排期。
- 值得抄的便宜项（非阻塞）：theherta4 的 `export_add_missing_vertex_groups`（导出时自动
  重排/填补数字顶点组编号空缺）——待确认我们导出是否已等价处理，缺则补。
- 若日后有余力再做：高风险热图叠加 + 一键进权重模式 / 聚类跳转。
...
### P4 — 生态扩展（~75%，**多组件已降优先级 / 暂缓**）
- ✅ Mod Manager 已功能完备并随 `3dmigoto-gkms-v0.5.x` 发布（扫描/启停/F10/冲突/完整性/
  路径记忆/封面/拖拽安装/Inno 安装包）；后续为增量打磨，非阻塞。
- ⚠️ **脸 / 头发 / 饰品等其他部位多组件、Shape Key/表情、LOD/多 Drawcall：优先级降低、暂缓。**
  架构上各自也是单 t0、可复用 body 的逆解链，随时能起，但不主动排期；接受"单 body 换装"为
  当前交付边界。若日后有明确需求（如头发 mod 呼声高）再单独启动，profile 已备好
  hairResource/faceResource、0.7.3 的 targets=游戏资源名也已就绪。

## 6. 与主流 Model Importer 的定位差异

GakumasMI 的独特价值不是作者工具完整度，而是解决了 GIMI/WWMI/EFMI 通常不面对的输入
条件：**从最终 CPU-skinned VB 反解每帧矩阵并重蒙皮自定义拓扑**。核心算法、作者蒙皮
（自动传权已超参考基准）、Mod Manager 均已到位。透明(P3)、蒙皮精修(P1)、多组件(P4) 均已
降优先级/暂缓 —— **项目在"单 body 换装 + Mod Manager"这个交付边界上已基本收敛**，仅剩材质
打磨(P2) 与回归维护(P0) 为低强度进行项；其余按需再启，不主动排期。
