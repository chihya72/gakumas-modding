...
| 模块 | 完成度 | 说明 |
|---|---:|---|
| GPU 核心（逆解每帧矩阵 + 重蒙皮 + 注入） | ~95% | 同源重建与游戏动态 VB0 逐字节一致，RMS ≈ 1e-6；任意拓扑跨服装实机跑通。已冻结、已审计，0.6.x/0.7.0 期间无变动。 |
| 配置档生成（一键：注入 + 结构 + 逆算子） | ~90% | 抓帧 + Body JSON 资源库一键产出；缺骨架时从 `m_BoneNameHashes` + `m_BindPose` 合成，500+ 套全可用。0.7.0 抓帧主 draw 选择更稳（优先匹配期望顶点数/可见贴图绑定数）。 |
| 蒙皮转权（作者模型 → 配置档骨架） | ~75% | 全身最近表面传权重稳定、自动限四权重/归一化、风险标记。手指/宽袖/裙摆仍需人工复核，缺可视化精修工具；0.6.x/0.7.0 无进展。 |
| 贴图 / 材质（t0 自动 DDS、分材质 t1/t4 烘焙、中性、COLOR/描边） | ~85% | t0/t1/t4 通道语义已实机实测并在 0.7.0 收敛（t4.rgb=暗面材质颜色版、t4.a=近似二值遮罩，非阴影强度灰阶）；新增 t1 单通道输入、旧 profile `shadeColor(ps-t2)`→`ps-t4` 自动迁移防错、`m_bdy`/`m_bdyco` 各自 t0 分离绑定不回退。缺多套服装持续校准、PNG→DDS 多材质 selector。 |
...
| 健壮性 / 回归测试 | ~75% | 数据契约审计（60/60）+ CI 跑的 6 个测试：buffer 打包契约、mod.ini 契约（含 0.7.0 新增旧 profile 迁移回归）、抓帧抽取（含 0.7.0 draw 选择回归）、逆解数值（合成）、材质烘焙（含 0.7.0 t1 通道覆盖/t4.A 预设回归）；逆解数值与契约审计另有本地数据档（见 P0）。 |
| 产品化（多组件 / Mod Manager / 发布） | ~40% | 仅 HSKI Body 单组件主线；脸/头发/饰品、Shape Key、LOD、Mod Manager 未做；0.6.x/0.7.0 无进展。 |

**已确认边界**：学马所有 body 都是**单 t0**，工具的单 t0 模型与游戏结构一致。多材质身体
导出**不需要**（MMD 移植版的多贴图属作者侧问题，应在 Blender 里合成单图）。

## 2. 插件当前能力（0.7.0 操作器一览）
...
- 材质属性：材质类型预设、`渲染材质`（`不透明` / `原生co NATIVE_CO`，0.6.2 起不再有自建
  `镂空`/`半透明`）、明暗微调；`t4.A` 不暴露为作者选项，由材质类型预设固定。
...
### P3 — 透明材质（60% → 看可行性）
- 镂空/cutout 子目标已用原生 `m_bdyco`（`NATIVE_CO`）验证完成，不再是本阶段重点；
- 剩余目标缩小为：找角色专属前向透明窄触发点，做玻璃/薄纱级真正连续半透明
  （`m_bdyco` 已证实是 cutout 而非半透明，详见透明专题第 3、5 节）；
- 若长期找不到安全触发点，需评估是否直接接受"仅镂空、不做真半透明"为最终边界。
