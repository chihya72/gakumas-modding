...
> 连模型都没试过。逐项验证等级见下方「逐项验证等级」一节——**它与本条警告互相矛盾，见该节末尾的说明**。
...
> ⚠ **这张表和本文开头那条警告互相矛盾，需要作者裁决。** 表说骨骼映射表单、承重关节闸门、
> 三步 UI 都是「A 实证」（有具体证据描述：从 RC ZIP 全新安装走完整流程、故意破坏 `Spine`
> 被拦后补齐放行、两套 Mod 已进游戏）；而 `README.md`、`CHANGELOG.md` 顶部和本文开头都写
> 「没有人在真 Blender 面板里点过」。后者写于 2026-07-27，早于 RC 闭环，**很可能是没跟着
> 更新的旧话**。在作者确认之前，对外文档保持保守说法不改——宁可少说，不可多说。
