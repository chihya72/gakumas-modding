...
| 4 | 新骨链的最低限度实机行为 | **未达到 A；下一开发重点** | 游戏中确认加载不崩、链条被注册，并取得可重复的画面级摆动案例 | P0 | 中 | 中 | 只有 `createdBones`/`swingPrepared` 日志；现役管线尚无稳定画面证据，旧原型结论不得沿用。**2026-08-09 新证据：现役导出的 sidecar `newBones` 是空的**（`chisaki-swimsuit`、`hmsz-fuyuko-icu` 均为 0），源专属骨没有摆动参数也没有链尾 tip，运行时逐根做成长度 1 的链、`RegisterBones` 抛异常。先补这个数据缺口再谈手感 |
