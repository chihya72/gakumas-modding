...
| `hmsz-fuyuko-icu` | `hmsz-cstm-0059` | **源专属新增骨与自建摇物**：bindPose/四元数/链根约定；2026-08-11 飘带与长链画面级通过 |

`hmsz-fuyuko-icu` 后来又成为**自建摇物链的第一个画面级成功案例**（2026-08-11），见下方
「自建摇物骨打通」。
...
| 4 | 新骨链的最低限度实机行为 | **A 实证（2026-08-11）** | 游戏中确认加载不崩、链条被注册，并取得可重复的画面级摆动案例 | P0 | 中 | 中 | `hmsz-fuyuko-icu`（`hmsz-cstm-0059`）飘带正常摆动、末端不抽、不穿模。排查经过与被推翻的旧结论见下方「2026-08-11」一节 |
...
| 健壮性 / 回归测试 | ~90% | 45 项 pytest 契约测试 + 4 个纯 Python 冒烟/回归脚本进入 CI；Blender 4.2.7/4.5.3 矩阵各跑安装、UI、材质烘焙、AB 导出 4 个闭环。测试不再依赖本机绝对路径、gitignored 抓帧或旧版 zip。 |
...
| 优先级 | 事项 | 完成条件 |
|---|---|---|
| — | ~~为自建摇物链取得稳定画面级案例~~ | **2026-08-11 已完成**，见下方「自建摇物骨打通」 |
| P0 | 回归与发布维护 | 45 项 pytest + 纯 Python 冒烟脚本 + Blender 4.2/4.5 四段闭环持续通过（移除 3DMigoto 后 mod.ini/传权两组测试已删） |
| — | ~~装饰骨默认策略翻转~~ | **已完成并实机确认**：位置匹配退出默认路径，改为跟源父骨 + `胸`→`Bust*_S` 名字规则；异常件由 override 兜底 |
...
排序依据：正式版主链和测试闭环已完成；自建摇物链已于 2026-08-11 画面级确认。
**hair t6 HHL 与 HairProp 分段是下一项重点。**ShapeKey、蓝图树和多游戏支持仍不是下一步。

## 2026-08-12：Runtime 协议与热开关集成状态

- 当前导出物的骨架 sidecar 必须带与 Runtime 匹配的 `runtimeProtocol` 和 `buildId`。旧插件包
  缺协议或跨构建混装时被拒绝完整替换属于预期保护；必须从原 `.blend` 用当前插件重导，不能
  手改字段冒充兼容。
- 已用 `hmsz-fuyuko-icu` 实机确认“管理页 ON 时当前没有目标 Renderer”的修复：Runtime 先记录
  `hotInstances=0` 并排队；返回主页后资源加载路径完成替换，生命周期 Hook 以
  `applied=0 alreadyPatched=1` 确认并清队列。该值表示“已经由另一条路径应用”，不是失败。
- 最新日志中 `atbm-cstm-0140` 只执行了 OFF，没有走“零活体后 ON”的分支，所以仍需单独复验；
  不能用 `hmsz` 的证据代替。
- 直接热 ON 可以刷新已有活体的 Mesh、材质、骨绑定与碰撞体，但新增摇物骨/链只在 prefab graft
  与角色初始化阶段注册；改变 swing 结构后仍需重新进入场景。

## 2026-08-11：自建摇物骨打通，✅ 已画面级确认

一轮实机排查下来，**原先写在文档里的判断几乎全是错的**，逐条推翻并修复。完整流水线见
[`ab-route-notes.md`](ab-route-notes.md) §10。

**被推翻的旧结论**：

| 旧结论 | 实况 |
|---|---|
| `newBones: []` 是导出器静默丢数据 | 7 个成品 `new_source_chain` 全是 0，`integrate` 一次都没被走过 |
| `UpdateChainInfo` 会排除每条链最后一根 | 链尾**在层里**（游戏自己的 `Skirt5_S_End` 也在） |
| `active` 是每帧 LOD 标志、刚建完必然 0 | 是**序列化授权字段**；同一帧游戏自己的链就是 1 |
| `radius`/`smoothing` 由 `UpdateChainInfo` 计算 | 它根本不算，新建层拿的是默认值 |
| `pendulum`/`pendulumRange`/`wind` 不是序列化字段 | 全是，逐骨授权，bundle 内嵌 typetree 直接可读 |
| `ActorSwingChain` 只有一条、挂 Pelvis（学马叫 Hips） | 那是偶像荣耀；学马多条链、宿主随部位走，且 `Hips ≠ Pelvis` |

**修掉的六处**：

1. **并行表越界** —— `initialTransforms` 与 `swingDynamicBones` 按下标并行，运行时只往后者
   追加就会长度失衡、`RegisterBones` 中途夭折。改为 **graft 时在 prefab 上建骨建链**，让游戏
   自己的 `Initialize()` 收走；删掉列表追加与 SEH 兜底。
2. **参数写全** —— 此前只写六项，`pendulumRange`（原版 84.6% 取 1.0，留 0 等于重力项失效）、
   `wind`、`limitInfo`、collider 全缺。基准表由 `tools/scan_vanilla_swing_bones.py` 扫 530 套
   原版 body 产出（bundle 内嵌 typetree，字段按名读，不是猜偏移）。
3. **飘带不该建链** —— 原版实测裙类 94% 挂链、飘带绳结类只有 2.6%。链是裙摆那层
   `around/radius` 环形碰撞专用的。
4. **角度限位不能照搬** —— 按骨轴授权，原版骨轴是 local −X、MMD 源是 −Z，照搬会锁死真·摆动
   轴。默认 `useLimit=0`，见 §11。
5. **`useWindGlobalForce` 写成数字** —— 运行时 nlohmann 抛 `type_error.302` → **整份 sidecar
   作废、骨架 graft 整个跳过**（表现是网格根本没换、只有贴图生效）。导出侧出真 bool，运行时
   两种都收。
6. **链是空壳** —— prefab 上 `AddComponent` 建的链只有 `rootBones`，`OnEnable` 不建层（游戏
   自己的链在 bundle 里就带着序列化好的 layers）。现在在 hook 里调 `UpdateChainInfo`，再按
   1539 条原版链的实测补 `active`（layer>0 置 1）和 `radius`（逐层递增）。

**✅ 2026-08-11 画面级确认（`hmsz-fuyuko-icu` / `hmsz-cstm-0059`）**：飘带正常摆动、长链末端
花边不再抽、右飘带不再穿腿。**这是自建摇物链的第一个画面级成功案例**，此前所有"跑通"的说法
都只有日志。日志侧同时全绿：骨 15/15 进 `swingDynamicBones`、并行表同长 238/238、参数活体
读回一致、链尾 4/4、两条链各 4/5 层且 `active`/`radius` 与原版同构。

**唯一剩下的是手感：摆动幅度偏小。** 属调参范围，不是缺陷。可用旋钮与原版分位数见
[`ab-route-notes.md`](ab-route-notes.md) §12。

## 历史：为自建摇物链取得稳定实机案例（2026-08-11 前的排查记录）

> ⚠️ 以下是 2026-08-11 排查之前的记录，其中的四条「下一步该查什么」**全部落空**（真因见
> 上一节）。保留仅为说明当时为何会误判，别照着它再查一遍。

**（当时）现役管线尚无稳定、可重复的画面级成功案例。**`atbm-0140` 只有日志
...
**下一步该查什么**（按可信度排序）—— ⚠️ **这四条 2026-08-11 全部证伪，尤其第 1 条把真病因
当成了"正常现象"放过。别照做**：
...
1. ❌ **先换判据。**排查当时把 `ActorSwing layer[N].active=0` 当成病因，但
   ...
   → **证伪**：`active` 是序列化授权字段，不是每帧 LOD 标志。打印游戏自己的链作对照这个
   建议本身是对的，做了才发现同一帧游戏的链是 1、我们的全是 0 —— 差别在授权不在时机。
