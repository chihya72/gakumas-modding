...
> **状态（插件 v0.9.3）**：插件只做 **AB（AssetBundle）路线**，3DMigoto 逆蒙皮的传权与导出
...
> 六家只按公开命名规范写成、没拿到对应模型，认不出来时走面板逐行点选。**当前最大的已知缺口
> 是自建摇物链的装饰件在游戏里不动**（成品用刚性/跟裙摆绕开）。逐项验证等级见
...
- 骨架以游戏原版那套为准：同名骨不改 bindpose（实测逐元素偏差 0）；源模型自带的装饰骨
  （蝴蝶结、缎带）可作为**源专属新增骨**由运行时按 sidecar 现场新建，物理仍是未解项；
